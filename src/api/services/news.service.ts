...
const createNew = async (body: any) => {
    const newUser = new ANew({link:body?.link,category:"string",title:body?.title || '' ,image:'string',shortDescription:body?.shortDescription || '',body:body?.body || body?.content || ''});
    return newUser.save()
}
