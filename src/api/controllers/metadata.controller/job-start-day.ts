...
import { JobStartDay } from '../../models'
import status from 'http-status'

export default {
  getAll: async function (_: Request, res: Response, next: NextFunction) {
    try {
      const jobBudgets = await JobStartDay.find({})
  ...
  getOne: async function (req: Request, res: Response, next: NextFunction) {
    try {
      const trade = await JobStartDay.findById(req.params.id)
  ...
  create: async function (req: Request, res: Response, next: NextFunction) {
    try {
      const { body } = req
      const newJobBudget = new JobStartDay({
        ...body,
      })
  ...
  update: async function (req: Request, res: Response, next: NextFunction) {
    try {
      const { body } = req
      const updatedJobBudget = await JobStartDay.findByIdAndUpdate(
        req.params.id,
        { ...body }
      )
