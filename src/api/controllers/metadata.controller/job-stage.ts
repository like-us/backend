...
export default {
  getAll: async function (_: Request, res: Response, next: NextFunction) {
    try {
      const jobBudgets = await JobStage.find({})
  ...
  getOne: async function (req: Request, res: Response, next: NextFunction) {
    try {
      const trade = await JobStage.findById(req.params.id)
  ...
  create: async function (req: Request, res: Response, next: NextFunction) {
    try {
      const { body } = req
      const newJobBudget = new JobStage({
        ...body,
      })
  ...
  update: async function (req: Request, res: Response, next: NextFunction) {
    try {
      const { body } = req
      const updatedJobBudget = await JobStage.findByIdAndUpdate(req.params.id, {
        ...body,
      })
